...
public class JavaScriptPublicAPI {
	...
	private static AudioStreamPlayer getAudioPlayer(int index) {
		...
		if (players.size() > index) {
			...
			Logger.getLogger("JavaScriptPublicAPI").info(LogUtils.prefix("PLAYER INDEX GET:  " + player.id));
		}else{
		    Logger.getLogger("JavaScriptPublicAPI").info(LogUtils.prefix("PLAYER INDEX GET: NULL PLAYER" ));
		} 
		return player;
	}
